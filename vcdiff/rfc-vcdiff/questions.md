...
The image's don't-care positions are invalid the same way — a nonzero size byte on a NOOP, or a nonzero mode byte on a NOOP, ADD, or RUN, is refused for the same reason: those bytes have exactly one well-formed value (the zero the default-table image holds), so a nonzero one is damage evidence, and the table check is the tripwire. The checks split homes by what each needs to know: `deserializeCodeTable` owns everything decidable from the 1536 bytes alone (width, type validity, unused-field zeros); the cache-dependent COPY-mode bound and the NOOP+NOOP announcement live with the table-build step, which knows the declared cache sizes.
