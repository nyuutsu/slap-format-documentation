...
**Permit it: apply the patch and surface a note — settled with some unease.** A remnant after the last *complete* window is tolerated, not fatal. The window stream fully determines the output; bytes past it are never read and break no claim the patch makes about itself, so refusing a patch that yields a correct target over bytes we don't touch would cost real working patches for nothing. The note says what it can — the recognizable shape when there is one (the field data is overwhelmingly a `FF FF FF FF` + zero-padding tail from one toolchain), otherwise at least the byte length. Create never emits such a tail; convert drops it (it isn't patch semantics) and notes the drop.
...
The note's wording and the apply/convert plumbing are implementation, not decided here.
