...
The bb-aps12.zip archive was successfully retrieved from the icequake.net mirror
of dextrose.com. Contents:

- `aps.txt` -- the spec document (3952 bytes, dated 1998-12-17)
- `n64aps.c` -- the apply tool, full C source (7159 bytes)
- `n64caps.c` -- the create tool, full C source (6897 bytes)
- `n64aps.exe`, `n64caps.exe` -- compiled DOS/Win32 binaries
- `BLACKBAG.NFO` -- ASCII art group nfo
- `readme.txt` -- includes the spec verbatim
- `file_id.diz`, `makefile`

This is the AUTHORITATIVE reference for APS N64. Where the spec text and the
source code disagree, the source code is correct (and below all references to
"the spec" mean the source unless explicitly stated).

## Identity

- **Full name**: APS (Advanced Patching System) v1.0, spec v1.2
- **Magic**: `APS10` (5 bytes)
- **Authors**: Silo and Fractal from Blackbag (N64 demoscene group)
- **Date**: July 13, 1998 (initial draft)
- **Tool**: bb-aps v1.2, distributed as `bb-aps12.zip` (~68KB) on dextrose.com
- **Contact**: silo@blackbag.org, fractal@blackbag.org
- **Spec document**: `aps.txt` inside `bb-aps12.zip`. Surviving copy on the
  UniPatcher wiki, with noted errata about byte offsets in the N64-specific header.

This format is **completely unrelated** to APS GBA despite sharing the `.aps`
extension and the first three ASCII bytes of the magic. APS N64 (`APS10`) is from
1998, uses byte replacement, and was made for N64 backup copiers. APS GBA (`APS1`)
is from 2010, uses XOR diffing, and was made for GBA hacks. They share nothing else.

## Why it exists

IPS uses 3-byte offsets, so it cannot address beyond 16MB. N64 cartridges go to
64MB (and the format claims support up to 2GB). APS N64 uses 4-byte offsets.
That's the entire motivation.

## The Blackbag scene

Blackbag was an N64 demoscene/homebrew group active in 1998, deeply embedded in the
Doctor V64 / CD64 backup copier community centered on dextrose.com. Known members:
Stumble, Ste, Silo, Shroomz, Nagra, McBain, Jovis, Fractal, Datawiz, Breakpoint,
Snake.

Notable releases:
- **Psychodelic** (March 1998) -- N64 demo, won 3rd place at Presence of Mind '98.
  Code by Ste, GFX by Fractal.
- **Game Boy Emulator** (March 1998) -- N64 GB emulator by Snake & McBain, won 2nd
  place at POM98.
- **Freek World Intro** -- another demo.

Silo and Fractal co-authored APS. Fractal did graphics for the demos; Silo's
specific demo role isn't documented.

## Format specification (from C source, NOT the spec text)

The spec text in `aps.txt` has typos in byte ranges. The C source is the source
of truth. Specifically, the spec says "BYTE 69-74: Pad" (6 bytes) and "BYTE 75-79:
Size" (5 bytes) but the actual code does 5 bytes pad and 4 bytes size.

### Standard header (57 bytes)

| Bytes | Field            | Notes                                    |
|-------|------------------|------------------------------------------|
| 0-4   | Magic            | ASCII `APS10`                            |
| 5     | Patch type       | 0 = Simple, 1 = N64-specific             |
| 6     | Encoding method  | 0 = Simple Encoding (only defined value) |
| 7-56  | Description      | 50 bytes, space-padded, NOT null-terminated |

### Type 1 (N64-specific) extended header (bytes 57-77)

| Bytes | Field            | Notes                                          |
|-------|------------------|------------------------------------------------|
| 57    | Image format     | 0 = Doctor V64 (byteswapped), 1 = CD64/Z64/Wc/SP (big-endian) |
| 58-59 | Cart ID          | 2 bytes, stored in Z64 byte order (big-endian) |
| 60    | Country code     | 1 byte                                         |
| 61-68 | CRC              | 8 bytes, stored in Z64 byte order (big-endian) |
| 69-73 | Padding          | 5 bytes (n64caps writes all zeros)             |
| 74-77 | Destination size | 4 bytes, signed long little-endian             |

### Type 0 (Simple) header

This type IS described in the spec, but `n64aps.c` line 83 says:
```c
if (PatchType != 1)
{
...
**The reference apply tool rejects type 0 patches.** And `n64caps.c` line 26:
```c
unsigned char PatchType=TYPE_N64;  // hardcoded
```
**The reference create tool only emits type 1.** Type 0 is documented but
unsupported by the reference tools. Other tools (RomPatcher.js, slap) added
type 0 support, but bb-aps is type-1-only.

### Records (encoding method 0)
...
|-------------|------|-------------------------------------------|
| Offset      | 4B   | uint32 LE                                 |
| Data length | 1B   | 1-255 = literal, 0 = RLE marker           |

If `data length > 0`: the next `data length` bytes are the literal replacement.

If `data length == 0`: this is an RLE record. Two more bytes follow:

| Field       | Size | Notes                |
|-------------|------|----------------------|
| Fill value  | 1B   | the byte to repeat   |
| Repeat count| 1B   | 1-255                |

### Termination

No EOF marker. Records are read until end of file. (Compare IPS, which uses an
`EOF` sentinel.)

### CRC / checksum

None over the patch data. Only the N64 cart header CRC is referenced (for source
validation in type-1 patches), and that's CRC of the N64 ROM, not of the patch.

### Reversibility

**No.** APS N64 is byte replacement: records contain new data, not old. Once
applied, the original bytes are gone. Same fundamental limitation as IPS.

## Limitations baked into the format

- **Per-record data length**: 1 byte = max 255 bytes of literal per record
- **Per-record RLE count**: 1 byte = max 255 bytes of fill per record
- **Offset width**: 4 bytes = 4GB address space (spec claims "up to 2GB")
- **Encoding methods**: only 0 defined, no extension mechanism actually used
- **Patch types**: only 0 and 1 defined
- **Description**: 50 bytes, space-padded, no encoding declared (system codepage)
- **Image format**: only V64 and Z64 distinguished; no other byte orderings
- **No patch checksum**: corruption is undetectable
...
## Files in /tmp/bb-aps12/

- `/tmp/bb-aps12/aps.txt` -- spec (with byte-range typos)
- `/tmp/bb-aps12/n64aps.c` -- apply tool source (authoritative)
- `/tmp/bb-aps12/n64caps.c` -- create tool source (authoritative)
- `/tmp/bb-aps12/n64aps.exe` -- compiled apply binary (DOS/Win32)
- `/tmp/bb-aps12/n64caps.exe` -- compiled create binary
- `/tmp/bb-aps12/readme.txt` -- includes spec verbatim plus dedication "to CZN"
- `/tmp/bb-aps12/BLACKBAG.NFO` -- group nfo with ASCII art
