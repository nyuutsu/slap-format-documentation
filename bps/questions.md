...
slap rejects it. The parser reads the last twelve bytes as the footer, computes `patch-checksum` over the rest, and the check fails. No special detection for "trailing bytes"; no heuristic for stripping.
