# APS (GBA) Format Research

## Identity

- **Full name**: Alternate Patching System (not "Advance Patch System")
- **Magic**: `APS1` (4 bytes, LE dword `0x31535041`)
- **Author**: HackMew (Andrea Sartori), Italian Pokemon ROM hacker
- **Tool**: A-Ptch ("Advance Ptch"), VB6, released January 21 2010
- **Website**: `www.andreasartori.net/hackmew` (defunct)
- **Scene**: PokeCommunity forums, GBA ROM hacking
- **Copyright string in binary**: `2010 HackMew`

The APS N64 format (`APS10` magic, by Silo & Fractal from Blackbag, 1998) is a
completely unrelated format that shares only the `.aps` extension and part of the
name. APS GBA is the common one; APS N64 is obscure.

## Format specification

Source: UniPatcher wiki (only known spec), confirmed against A-Ptch source.
...
|--------------------|------|-------------|
| Magic              | 4B   | `APS1`      |
| Original file size | 4B   | uint32 LE   |
| Modified file size | 4B   | uint32 LE   |

### Records (repeated to EOF)

Each record is exactly 65544 bytes:

| Field                          | Size   | Encoding    |
|--------------------------------|--------|-------------|
| Offset                         | 4B     | uint32 LE   |
| CRC16 of original 64KB block   | 2B     | uint16 LE   |
| CRC16 of modified 64KB block   | 2B     | uint16 LE   |
| XOR data (original ^ modified) | 65536B | raw bytes   |

Total file size is always `12 + (N * 65544)`.

### CRC16

CRC16-CCITT, polynomial `0x1021`, initial value `0xFFFF`, computed over a full
65536-byte buffer. Short blocks (at end of file) are zero-padded.

### Diff method

Pure XOR. `patch_data[i] = original[i] ^ modified[i]`. XOR is self-inverse.

### Reversibility

The format is inherently bidirectional:
- XOR is its own inverse
- Both source and target CRC16s are stored per block
- Both source and target file sizes are stored in the header

Direction is auto-detected at apply time: compute CRC16 of the input block before
XOR, check which stored CRC it matches. If it matches the original CRC, this is a
forward apply; if it matches the modified CRC, this is an undo.
